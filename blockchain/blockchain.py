...
class Blockchain:
    ...
    # def create_merkel_root(self):
    #     queue = deque()
    #     for ele in self.transactions:
    #         queue.append(ele['transaction'])
    #
    #     if len(queue) == 0:
    ...
    #         n = len(queue)
    #         if n % 2 != 0:
    #             queue.append(hashlib.sha256('0'.encode()).hexdigest())
    #
    #         for i in range(0, n, 2):
    #             ele1 = self.hash(queue.popleft())
    #             ele2 = self.hash(queue.popleft())
    #             queue.append(hashlib.sha256((str(ele1) + str(ele2)).encode()).hexdigest())
    #     return queue[0]
...
#         self.transactions = []
#         self.chain = []      # contain the list of blocks
#         # creating genesis block
#         # nounce =0 ,prev_hash ='00
#         self.create_blocks(0, '00')
#
#     def create_blocks(self, nounce, prev_hash):
...
#
# # --------------Instantiate the Ndde-----------------------------
#
# app = Flask(__name__)
#
# @app.route("/")  #listen request to an any request from this slash ("/")
# def hello_world():
#     # return "./index.html"   -----> return string
#     return render_template("./index.html")  # return file
# # -------------------------------------------
# if __name__  == '__main__':
#     from argparse import  ArgumentParser
#     parser_1 = ArgumentParser()
#     parser_1.add_argument('-p', '--port', default=5001, type=int, help="port to listen to")
#     argument_1 = parser_1.parse_args()
#     port = argument_1.port
#
#     app.run(host='127.0.0.1', port=port, debug=True)
